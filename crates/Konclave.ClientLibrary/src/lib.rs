...
impl PlaceholderName {
    ...
    /// # Errors
    ///
    /// Returns [KonclaveClientLibraryError] when the supplied value is blank or too long.
    pub fn parse(value: impl Into<String>) -> Result<Self, KonclaveClientLibraryError> {
        let candidate = value.into();
        let trimmed = candidate.trim();

        if trimmed.is_empty() {
            return Err(KonclaveClientLibraryError::EmptyValue);
        }

        if trimmed.len() > MAX_NAME_LEN {
            return Err(KonclaveClientLibraryError::ValueTooLong {
                max: MAX_NAME_LEN,
                actual: trimmed.len(),
            });
        }
...
pub enum KonclaveClientLibraryError {
...
mod tests {
    ...
    fn rejects_empty_placeholder() {
        assert_eq!(
            ...
            KonclaveClientLibraryError::EmptyValue
        );
    }
    ...
    fn rejects_overlong_placeholder() {
        ...
        assert_eq!(
            ...
            KonclaveClientLibraryError::ValueTooLong {
                max: MAX_NAME_LEN,
