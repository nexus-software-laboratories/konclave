...
| Copilot tool escapes collaboration policy | Keep a turn-scoped pre-tool gate, preload only `send_message` so the permitted effect does not depend on deferred discovery, omit the hook-only authorization from its model-facing schema, reject caller-supplied authorization, accept only a bounded object or serialized object with the exact caller field allowlist, correlate fresh interactive and delivery connections through their authenticated session public key, bind and inject a one-use authorization for the exact request, handling attempt, response arguments, active digest, live delivery consumer, and bounded expiry, verify those conditions in the atomic handling/outbox reservation, and deny external, unknown, approval-required, cross-conversation, or request-creating tools |
