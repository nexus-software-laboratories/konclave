...
| Trusted-device alias substitution or rollback | Seal alias, canonical device identifier, and exact root together under profile-and-device associated data; enforce one alias per active root, authenticate an independent row count, reject deletion or cross-profile substitution, and resolve only against current root-verified membership |
