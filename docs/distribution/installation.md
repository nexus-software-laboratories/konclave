...
For the `v0.1.2` prerelease, a clean machine with GitHub CLI and PowerShell can
...
gh release download v0.1.2 \
  ...
  --dir konclave-0.1.2
pwsh ./konclave-0.1.2/Verify-Release.ps1
```
...
installer files. The `v0.1.2` installer can still verify, extract, supervise, and
...
```shell
docker image load --input konclave-community-relay-container-0.1.2-linux-amd64.docker.tar
...
```shell
docker image load --input konclave-a2a-gateway-container-0.1.2-linux-amd64.docker.tar
```
