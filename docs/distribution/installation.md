...
For the `v0.1.13` prerelease, a clean machine with GitHub CLI and PowerShell can
...
gh release download v0.1.13 \
  ...
  --dir konclave-0.1.13
pwsh ./konclave-0.1.13/Verify-Release.ps1
```
...
installer files. The `v0.1.13` installer can still verify, extract, supervise, and
...
```shell
docker image load --input konclave-community-relay-container-0.1.13-linux-amd64.docker.tar
