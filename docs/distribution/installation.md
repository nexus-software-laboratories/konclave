...
For the `v0.1.5` prerelease, a clean machine with GitHub CLI and PowerShell can
...
gh release download v0.1.5 \
  ...
  --dir konclave-0.1.5
pwsh ./konclave-0.1.5/Verify-Release.ps1
```
...
installer files. The `v0.1.5` installer can still verify, extract, supervise, and
...
```shell
docker image load --input konclave-community-relay-container-0.1.5-linux-amd64.docker.tar
